...
Message = (function() {
  function Message(_client, data) {
    ...
    this.getReactions = __bind(this.getReactions, this);
    this._onReact = __bind(this._onReact, this);
    this.unreact = __bind(this.unreact, this);
  ...
  Message.prototype.deleteMessage = function() {
    var params, util;
    if (this.type === 'reaction_added') {
      util = require('util');
      console.log(JSON.stringify(this));
      if (this.item != null) {
        params = {
          timestamp: this.item.ts,
          channel: this.item.channel,
          name: this.reaction
        };
        this._client.logger.debug("Sending reation delete request");
        this._client.logger.debug(params);
        return this._client._apiCall("reactions.remove", params, this._onDeleteMessage);
      }
    } else {
      if (this.ts) {
        params = {
  ...
  Message.prototype.react = function(emoji_name) {
    var params;
    params = {
      timestamp: this.ts,
      channel: this.channel,
    ...
    if (this.ts) {
      this._client.logger.debug("Reacting to message");
      this._client.logger.debug(params);
      return this._client._apiCall("reactions.add", params, this._onReact);
    }
  };

  Message.prototype.unreact = function(emoji_name) {
    var params;
    params = {
      timestamp: this.ts,
      channel: this.channel,
    ...
    if (this.ts) {
      this._client.logger.debug("Removing reaction to message");
      this._client.logger.debug(params);
      return this._client._apiCall("reactions.remove", params, this._onReact);
    }
  };

  Message.prototype._onReact = function(data) {
    return this._client.logger.debug(data);
  };

  Message.prototype.getReactions = function(full, callback) {
    var params;
    if (full == null) {
      full = false;
    }
    ...
    if (this.ts) {
      this._client.logger.debug("Getting reactions to message");
      this._client.logger.debug(params);
      return this._client._apiCall("reactions.get", params, callback);
    }
  };
